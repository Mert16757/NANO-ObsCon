...
#define SQM_LIMIT 13.25   // Calibration factor to establish against ASTAP values or with SQM-L
#define Pedestal 0
  ...
  if (Serial.available() >0 ){
    ...
    switch ( Query ){
      ...
      case 1:
        ...
        Serial.flush();
        if ( First_Connect == 0 ) {
          last_SQM_val = sqm();    // We read the value of SQM because it can take up to 35 seconds when very dark
        }
        break;
      case 2:
...
double sqm() {
  ...
  while (( micros() -start ) < 5000000 )    // semi fixed time of 5 seconds depending on the 2 while
  {
